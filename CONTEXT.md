...
**受管媒体（managed media）**：
项目与全局资产中由对象存储管理的图片、音频、视频及其缩略图、宫格、最终输出和版本媒体。`project.json`、剧本、源文档和运行时文件不属于受管媒体，继续作为本地项目数据保存。

**七牛媒体存储（Qiniu media storage）**：
由 `QINIU_ENABLED` 控制的私有 Kodo 主存储模式。关闭时受管媒体按原有本地文件系统处理；开启时，生成、上传、版本还原和项目导入必须先完成七牛上传与 ETag 校验，才能提交对应的任务结果或项目元数据。缺失配置、上传失败或下载失败必须显式失败，不能以本地文件作为静默降级播放源。下载域名必须使用 HTTPS，凭据只存在部署密钥管理或运行环境，不能进入代码、数据库、日志或项目文件。

**对象键（object key）**：
受管项目媒体映射为 `<QINIU_OBJECT_PREFIX>/<项目名>/<项目内相对路径>`；全局资产映射为 `<QINIU_OBJECT_PREFIX>/_global_assets/<相对路径>`，前缀默认 `projects`。项目记录始终保存项目内相对路径，绝不持久化会过期的七牛签名 URL；供应商返回的 `video_uri` 保留其原有语义。

**客户端媒体访问（client media access）**：
客户端仍访问既有文件 API。后端先完成认证和路径安全校验，在七牛启用时返回 `307` 到短期私有签名 URL；本地文件只服务后端处理，不是客户端媒体访问的回退来源。

**按需物化缓存（materialization cache）**：
ffmpeg、导出和其他需要本地文件的后端流程，按对象级锁从七牛下载到原项目相对路径；缓存以受控 LRU 管理，避免并发重复下载。临时导入目录不进入缓存索引。

**项目归档、删除与迁移（archive import, deletion and migration）**：
导出前会物化缺失的云端媒体；导入 ZIP 时先校验并解压，随后以最终项目名上传全部受管媒体，成功后才安装本地项目。云端存储启用时禁止覆盖导入已存在的项目，必须自动重命名，或先删除原项目再导入，避免新旧媒体对象混写。删除项目时先删除其云端对象前缀，云端删除失败则保留本地项目供重试。导入 ZIP 本身是临时请求文件，不作为对象保存。历史本地媒体通过迁移脚本分批上传、断点续跑和 ETag 校验；默认 dry-run，未确认前不删除云端对象，删除本地缓存必须显式启用。
