...
**受管项目文件（managed project file）**：
项目目录中的业务文件，包括 `project.json`、源文档、剧本、草稿、图片、音频、视频、缩略图、宫格、最终输出和版本数据。数据库、用户/所有权、任务、Agent Profile、隐藏锁、检查点、缓存索引和临时文件属于部署控制面或运行时文件，不进入对象存储。

**七牛项目存储（Qiniu project storage）**：
由 `QINIU_ENABLED` 控制的私有 Kodo 主存储模式。关闭时项目按原有本地文件系统处理；开启时，业务文件写入必须完成七牛上传与 ETag 校验后才能返回成功。本地非媒体文件是供 Agent 和同步代码使用的工作副本，媒体是受 LRU 管理的按需物化缓存。缺失配置、上传失败或下载失败必须显式失败，不能静默回退为仅本地成功。当前并发模型为单写实例，不支持多个后端同时写同一项目。

**对象键（object key）**：
受管项目文件映射为 `<QINIU_OBJECT_PREFIX>/<项目名>/<项目内相对路径>`；全局资产映射为 `<QINIU_OBJECT_PREFIX>/_global_assets/<相对路径>`，前缀默认 `projects`。项目记录始终保存项目内相对路径，绝不持久化会过期的七牛签名 URL；供应商返回的 `video_uri` 保留其原有语义。

**客户端媒体访问（client media access）**：
客户端媒体仍访问既有文件 API。后端先完成认证、项目所有权和路径安全校验，在七牛启用时返回 `307` 到短期私有签名 URL；`project.json`、剧本等结构化数据继续由认证业务 API 返回。本地文件只服务后端处理，不是客户端媒体访问的回退来源。

**按需物化缓存（materialization cache）**：
项目首次激活时按 ETag 恢复或刷新非媒体工作副本；ffmpeg、导出和其他需要媒体文件的后端流程，按对象级锁从七牛下载到原项目相对路径。仅媒体缓存进入受控 LRU，临时导入目录不进入缓存索引。

**项目归档、删除与迁移（archive import, deletion and migration）**：
导出前会物化完整项目业务文件；导入 ZIP 时先校验并解压，随后以最终项目名上传全部受管项目文件，成功后才安装本地项目。云端存储启用时禁止覆盖导入已存在的项目，必须自动重命名，或先删除原项目再导入，避免新旧对象混写。删除单个业务文件或整个项目时先删除云端对象，云端删除失败则不确认本地删除成功。导入 ZIP 本身和临时导出包不作为对象保存。历史本地项目通过迁移脚本分批上传、断点续跑和 ETag 校验；默认 dry-run，`--delete-local` 只清理已验证的媒体缓存，非媒体工作副本继续保留。
